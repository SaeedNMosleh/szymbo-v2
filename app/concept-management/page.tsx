...
export default function ConceptManagementPage() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'explorer' | 'map' | 'hierarchy' | 'bulk' | 'import'>('dashboard');
  ...
  const [llmResults, setLlmResults] = useState<LLMExplorationResult | null>(null);
  ...
  const [selectedTagsForCategory, setSelectedTagsForCategory] = useState<string[]>([]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const conceptsToAdd = concepts
          .filter(c => c.tags.some(tag => selectedTagsForCategory.includes(tag)))
          .map(c => c.id);
  ...
  const renderDashboard = () => (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-lg">
        <h1 className="text-3xl font-bold mb-2">Concept Management Hub</h1>
        <p className="text-blue-100">
          Comprehensive concept organization, visualization, and intelligent management system
        </p>
      </div>
          ...
          {!categoryCreationMode ? (
            <div className="space-y-4">
              <p className="text-gray-600">
                Group concepts into meaningful categories based on their tags. This helps organize related concepts automatically.
              </p>
              
              {stats && stats.topTags.length > 0 && (
                <div>
                  <h4 className="font-medium mb-2">Most Popular Tags</h4>
            ...
            <div className="space-y-4">
                  ...
                  {allTags.map(tag => (
                    <label key={tag} className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedTagsForCategory([...selectedTagsForCategory, tag]);
                          } else {
                            setSelectedTagsForCategory(selectedTagsForCategory.filter(t => t !== tag));
                          }
                        }}
                      />
              ...
              {selectedTagsForCategory.length > 0 && (
                <div>
                  <label className="block text-sm font-medium mb-2">
                    Concepts that will be included ({concepts.filter(c => c.tags.some(tag => selectedTagsForCategory.includes(tag))).length})
                  </label>
                  <div className="text-sm text-gray-600 max-h-24 overflow-y-auto">
                    {concepts
                      ...
                      .map(c => c.name)
                      .join(', ')}
                    {concepts.filter(c => c.tags.some(tag => selectedTagsForCategory.includes(tag))).length > 10 && '...'}
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  disabled={!newCategoryName || selectedTagsForCategory.length === 0}
                >
                  Create Category
                </Button>
                <Button 
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <p className="text-gray-600">
            Use natural language to explore concepts beyond just tags. Ask about relationships, similarities, or get recommendations.
          </p>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">Exploration Query</label>
              <Textarea
                value={llmQuery}
          ...
          {llmResults && (
            <div className="mt-6 space-y-4">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-blue-700 text-sm mb-2">{llmResults.categoryAnalysis}</p>
                <p className="text-blue-700 text-sm">{llmResults.difficultyAnalysis}</p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {llmResults.relatedConcepts.map((concept) => (
                    <div key={concept.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                      <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <Badge variant={selectedConcept.category === 'grammar' ? 'default' : 'secondary'}>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="capitalize">{selectedConcept.sourceType}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div>
                ...
                {selectedConcept.vocabularyData && (
                  <div className="mt-4">
                    <h4 className="font-medium mb-2">Vocabulary Data</h4>
                    <div className="space-y-1 text-sm">
                      <div><span className="font-medium">Word:</span> {selectedConcept.vocabularyData.word}</div>
                      <div><span className="font-medium">Translation:</span> {selectedConcept.vocabularyData.translation}</div>
                      <div><span className="font-medium">Part of Speech:</span> {selectedConcept.vocabularyData.partOfSpeech}</div>
                      {selectedConcept.vocabularyData.gender && (
                        <div><span className="font-medium">Gender:</span> {selectedConcept.vocabularyData.gender}</div>
                      )}
                    </div>
                  </div>
                )}
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {selectedConcepts.length > 0 && (
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => setSelectedConcepts([])}>
