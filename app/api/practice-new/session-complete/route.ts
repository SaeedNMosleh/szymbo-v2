...
export async function POST(request: NextRequest) {
  try {
    ...
    const { sessionId, userId, completionReason, totalTimeSpent } =
    ...
    const averageResponseTime =
      completedQuestions.length > 0
        ? completedQuestions.reduce(
            (sum: number, resp: IQuestionResponse) => sum + resp.responseTime,
            0
    ...
    const sessionAnalysis = {
      ...
      conceptsReviewed: [
        ...new Set(
          session.questionResponses.flatMap((resp: IQuestionResponse) =>
            session.questionsUsed.includes(resp.questionId)
      ...
      attemptsBreakdown: {
        firstAttempt: session.questionResponses.filter(
          (resp: IQuestionResponse) => resp.isCorrect && resp.attempts === 1
        ).length,
        secondAttempt: session.questionResponses.filter(
          (resp: IQuestionResponse) => resp.isCorrect && resp.attempts === 2
        ).length,
        thirdAttempt: session.questionResponses.filter(
          (resp: IQuestionResponse) => resp.isCorrect && resp.attempts === 3
        ).length,
        failed: session.questionResponses.filter(
          (resp: IQuestionResponse) => !resp.isCorrect && resp.attempts >= 3
        ).length,
      },
...
function generateRecommendations(
  ...
  if (completionReason === "abandoned") {
    recommendations.push(
      "Consider shorter practice sessions to build consistency"
    );
  ...
  if (analysis.accuracy < 50) {
    ...
    recommendations.push(
      "Consider drilling weak concepts with targeted practice"
    );
  } else if (analysis.accuracy > 80) {
    recommendations.push(
      "Great job! You're ready for more challenging concepts"
    );
    recommendations.push(
      "Consider adding new concepts to your practice rotation"
    );
  }

  if (analysis.attemptsBreakdown.failed > 0) {
    recommendations.push(
      "Review the concepts from questions you couldn't answer"
    );
  ...
  if (analysis.questionsAttempted < 5) {
    recommendations.push(
      "Try to complete more questions in each session for better progress"
    );
  }
...
// Interface for concept progress summary
interface ConceptProgressSummary {
  conceptId: string;
  masteryLevel: number;
  successRate: number;
  nextReview: Date;
