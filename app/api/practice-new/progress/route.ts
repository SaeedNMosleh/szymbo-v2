...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sessionId) {
      ...
      console.log(`Session ${sessionId}: Concept ${conceptId} - ${isCorrect ? 'Correct' : 'Incorrect'}`);
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (conceptId) {
      ...
      const progress = await SRSCalculator.initializeConceptProgress(conceptId, userId);
