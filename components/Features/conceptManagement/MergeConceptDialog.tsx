...
export function MergeConceptDialog({
  ...
  const [currentStep, setCurrentStep] = useState<"validation" | "preview">(
    "validation"
  );
  ...
  const prepareEnhancedMergePreview = useCallback(
    (fullConcepts: IConcept[]): MergePreviewData => {
      const [primary, ...secondary] = fullConcepts;

      // Aggregate all examples
      const allExamples = fullConcepts.flatMap((c) => c.examples || []);
      const uniqueExamples = [
        ...new Set(allExamples.filter((ex) => ex.trim() !== "")),
      ];
  ...
  const loadFullConceptData = useCallback(async () => {
    ...
    try {
      ...
      const conceptPromises = selectedConcepts.map(async (concept) => {
        ...
        if (!response.ok)
          throw new Error(`Failed to fetch concept ${concept.id}`);
  ...
  }, [selectedConcepts, prepareEnhancedMergePreview]);
  ...
  }, [isOpen, selectedConcepts, loadFullConceptData]);
  ...
  if (currentStep === "preview" && mergePreview) {
    const initialData: Partial<ConceptEditFormData> = {
      ...
      suggestedDifficulty: mergePreview.mergedPreview
        .difficulty as QuestionLevel,
