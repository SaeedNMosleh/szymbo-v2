...
export type {
  ...
  ParsedLLMResponse,
  LLMServiceMetrics,
  LLMProvider, // Added here as a type export
} from "./types";

// Convenience exports for common usage patterns
export const createOpenAIService = (
  config?: Partial<import("./types").LLMServiceConfig>
) =>
  import("./llmServiceFactory").then(({ LLMServiceFactory }) =>
    LLMServiceFactory.getOpenAIService(config)
  );

export const createGroqService = (
  config?: Partial<import("./types").LLMServiceConfig>
) =>
  import("./llmServiceFactory").then(({ LLMServiceFactory }) =>
    LLMServiceFactory.getGroqService(config)
  );

export const getAvailableLLMService = (
  preferredProvider?: import("./types").LLMProvider
) =>
  import("./llmServiceFactory").then(({ LLMServiceFactory }) =>
    LLMServiceFactory.getAvailableService(preferredProvider)
  );
...
export const getLLMService = (
  ...
  import("./llmServiceFactory").then(({ LLMServiceFactory }) =>
    LLMServiceFactory.getService(provider, config)
  );
